...
class Robot:
    ...
    def __init__(self):
        ...
        ## added by vasilis, latched so i can take the last position of each servo at anytime
        self.pub_servo1 = rospy.Publisher("servo/last_position/1", Int16, queue_size=10,latch=True)
        self.pub_servo2 = rospy.Publisher("servo/last_position/2", Int16, queue_size=10,latch=True)
    ...
    def set_servo_angle_def(self, servo, angle):
...
#       ANGLE_RANGE = [44, 122]
#       CENTER = 1550
#       logic is that at 90 degrees pulse is 1550 , 1550/90 = 17 , so 17 * angle = target step
#       also added some conditions to avoid physical limits like USB cable ;)
#       don't blame me, it works..
        pulse_range = [1000,2000]
        step = 17.2
        dst = int(step * angle)
        if dst < min(pulse_range):
            dst = min(pulse_range)
        elif dst > max(pulse_range):
            dst = max(pulse_range)
        else:
            dst = dst
        self.g.set_servo(servo,dst)
        servo_names = {self.S1: "servo1", self.S2: "servo2"}
        if servo == 1:
            self.pub_servo1.publish(angle)
        if servo == 2:
            self.pub_servo2.publish(angle)
        self.pub_joints.publish(JointState(name=[servo_names[servo]], position=[angle]))
...
#        print("new_angle2", new_angle2)
        ...
        odom = Odometry(header=Header(stamp=rospy.Time.now(), frame_id="odom"), child_frame_id="base_link", pose=self.pose, twist=twist)

#        transform = TransformStamped(header=Header(stamp=rospy.Time.now(), frame_id="world"), child_frame_id="base_link")
